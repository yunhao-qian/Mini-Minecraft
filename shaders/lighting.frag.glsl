...
DepthMapResult sampleDepthMap(vec2 textureCoords, int cascadeIndex, float shadowViewSpaceZ)
{
    ...
    averageDepthDifference = clamp(averageDepthDifference, 2.0, 100.0);
    ...
    result.depthSquared = 0.0;
    {
        ...
        vec2 blurRadius = u_shadowMapDepthBlurScales[cascadeIndex] * averageDepthDifference;
        for (int i = 0; i < PoisonSampleCount; ++i) {
            vec2 offset = PoissonDisk[i] * blurRadius;
            vec3 sampleCoords = vec3(textureCoords + offset, float(cascadeIndex));
